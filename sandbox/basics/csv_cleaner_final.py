...
def detect_dialect(csv_path: Path) -> csv.Dialect:
    """Sniff delimiter/quote rules from a small sample; default to comma."""
...
def read_rows(csv_path: Path) -> list[list[str]]:
    """Load all rows as lists using detected dialect."""
...
def clean_rows(rows: list[list[str]]) -> tuple[list[list[str]], dict[str, int]]:
    """Strip whitespace, drop empty rows; return cleaned rows and stats."""
...
def write_rows(csv_path: Path, rows: list[list[str]]) -> None:
    """Write rows with comma delimiter; empty input creates empty file."""
    if not rows:
...
def main() -> None:
    """End-to-end: read → clean → write; print basic stats."""
